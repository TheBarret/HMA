...
class UnifiedAnalyzer:
    ...
    def run(self, image_path: str, georeferencing: dict = None) -> AnalyzedTerrain:
        ...
        print(f"Min elevation: {heightmap.data.min():.1f}m")
        print(f"Max elevation: {heightmap.data.max():.1f}m")
    ...
    def visualize(self, analyzed: AnalyzedTerrain, graphs: dict = None, save_path: str = None):
        """
        Create consolidated visualization with tactical map symbols and relational networks.
        
        Args:
            analyzed: AnalyzedTerrain object from Layer 5
            graphs: Optional dictionary containing visibility_graph and connectivity_graph
                    from Layer 4 Relational analysis
        ...
        for peak in analyzed.peaks:
            x, y = peak.centroid
            ax_features.plot(x, y, '^', markersize=8, color='red', 
                            markeredgecolor='white', markeredgewidth=0.5, label='_nolegend_')
        
        ax_features.set_title(f'Features: Ridges (red), Valleys (blue), Peaks (▲)', 
                              fontsize=10, fontweight='bold')
        ax_features.axis('off')
        
        # ----- RIGHT: Final Analyzed Terrain (Tactical Map Symbols) -----
        ax_main.imshow(z, cmap='terrain', alpha=0.6)
        
        # ===== RELATIONAL NETWORK OVERLAYS =====
        # Use transparent edges to show relationships between features
        
        # Build feature ID mapping for quick lookup
        feature_by_id = {}
        for feature in analyzed.peaks + analyzed.ridges + analyzed.valleys + analyzed.saddles + analyzed.flat_zones:
            feature_by_id[feature.feature_id] = feature
        
        # 1. Draw visibility graph (if provided) - Light cyan, dashed, semi-transparent
        if graphs and 'visibility_graph' in graphs:
            vis_graph = graphs['visibility_graph']
            vis_edges = set()
            
            for f1_id, connected in vis_graph.items():
                if f1_id not in feature_by_id:
                    continue
                f1 = feature_by_id[f1_id]
                x1, y1 = f1.centroid
                
                for f2_id in connected:
                    # Avoid duplicate edges
                    edge = tuple(sorted([f1_id, f2_id]))
                    if edge in vis_edges:
                        continue
                    vis_edges.add(edge)
                    
                    if f2_id not in feature_by_id:
                        continue
                    f2 = feature_by_id[f2_id]
                    x2, y2 = f2.centroid
                    
                    # Draw visibility edge with transparency
                    ax_main.plot([x1, x2], [y1, y2], 'c-', linewidth=1.5, alpha=0.3, 
                               linestyle=':', label='_nolegend_')
            
            # Add legend entry for visibility edges
            visibility_patch = plt.Line2D([0], [0], color='cyan', linewidth=1.5, 
                                          linestyle=':', alpha=0.5, label='Visibility (LOS)')
        else:
            visibility_patch = None
        
        # 2. Draw connectivity graph (if provided) - Green, solid, semi-transparent
        if graphs and 'connectivity_graph' in graphs:
            conn_graph = graphs['connectivity_graph']
            conn_edges = set()
            
            for f1_id, connected in conn_graph.items():
                if f1_id not in feature_by_id:
                    continue
                f1 = feature_by_id[f1_id]
                x1, y1 = f1.centroid
                ...
                for f2_id in connected:
                    ...
                    # Draw connectivity edge with transparency
                    ax_main.plot([x1, x2], [y1, y2], 'g-', linewidth=1.8, alpha=0.35, 
                               label='_nolegend_')
            
            # Add legend entry for connectivity edges
            connectivity_patch = plt.Line2D([0], [0], color='green', linewidth=1.8, 
                                            alpha=0.5, label='Traversable Path')
        else:
            connectivity_patch = None
        
        # 3. Draw flow network (if provided) - Blue arrows, semi-transparent
        if graphs and 'flow_network' in graphs:
            flow_net = graphs['flow_network']
            
            for f1_id, downstream_list in flow_net.items():
                if f1_id not in feature_by_id:
                    continue
                f1 = feature_by_id[f1_id]
                x1, y1 = f1.centroid
                
                for f2_id in downstream_list:
                    if f2_id not in feature_by_id:
                        continue
                    f2 = feature_by_id[f2_id]
                    x2, y2 = f2.centroid
                    ...
                    # Draw the line
                    ax_main.plot([x1, x2], [y1, y2], 'b-', linewidth=1.2, alpha=0.4,
                               label='_nolegend_')
                    
                    # Draw arrowhead at 70% of the line (to avoid overlap)
                    arrow_pos = 0.7
                    ...
                    arrow_left = (arrow_x - arrow_length * np.cos(angle + arrow_angle),
                                 arrow_y - arrow_length * np.sin(angle + arrow_angle))
                    arrow_right = (arrow_x - arrow_length * np.cos(angle - arrow_angle),
                                  arrow_y - arrow_length * np.sin(angle - arrow_angle))
                    
                    ax_main.plot([arrow_x, arrow_left[0]], [arrow_y, arrow_left[1]], 
                               'b-', linewidth=1, alpha=0.5)
                    ax_main.plot([arrow_x, arrow_right[0]], [arrow_y, arrow_right[1]], 
                               'b-', linewidth=1, alpha=0.5)
            
            # Add legend entry for flow network
            flow_patch = plt.Line2D([0], [0], color='blue', linewidth=1.2, alpha=0.5,
                                   label='Water Flow Direction')
        else:
            flow_patch = None
        
        # ===== TACTICAL FEATURE SYMBOLS =====
        ...
        # Build legend with tactical symbols and relational networks
        legend_elements = [
        ...
        # Add relational network legend entries if they exist
        if visibility_patch:
            legend_elements.append(visibility_patch)
        if connectivity_patch:
            legend_elements.append(connectivity_patch)
        if flow_patch:
            legend_elements.append(flow_patch)
        ...
        if graphs:
            ...
            if 'visibility_graph' in graphs:
                vis_edges = sum(len(v) for v in graphs['visibility_graph'].values()) // 2
                stats_text += f"• Visibility: {vis_edges} LOS links\n"
            if 'connectivity_graph' in graphs:
                conn_edges = sum(len(v) for v in graphs['connectivity_graph'].values()) // 2
                stats_text += f"• Traversable: {conn_edges} paths\n"
            if 'flow_network' in graphs:
                flow_edges = sum(len(v) for v in graphs['flow_network'].values())
                stats_text += f"• Flow: {flow_edges} connections"
            
            ax_main.text(0.02, 0.02, stats_text, transform=ax_main.transAxes,
                        fontsize=8, verticalalignment='bottom',
                        bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
        
        ax_main.set_title('TACTICAL TERRAIN ANALYSIS\nMilitary Map Symbology + Relational Networks', 
                          fontsize=12, fontweight='bold')
        ax_main.axis('off')
        
        plt.suptitle(f'Profile: {self.config.game_type.value.upper()}', 
...
def main():
    """Run unified test."""
    image_path = "heightmap_large.png"
